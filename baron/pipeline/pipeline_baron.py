...
@follows(mkdir('s2-expression.dir'), runStar)
...
@transform(runLimma,
           regex(r'.*/(.*)-limma.txt'),
		   r's4-enrichr.dir/\1-t-listids.json')
...
def runEnrichr(infile, outfile):
	...
	# Read dataframe
	limma_dataframe = pd.read_table(infile, index_col='gene_symbol').sort_values(outfile.split('-')[-2], ascending=False)
