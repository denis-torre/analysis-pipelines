...
plot_auc <- function(infile, outfile, plot_type) {
    ...
    # Plot
    if (plot_type == 'violin') {
        gp <- ggplot(auc_dataframe, aes(x=Similarity, y=auc, fill=Similarity)) + geom_violin() + facet_wrap(library~., scales='free_y', ncol=3) + theme_minimal() + scale_y_continuous(lim=c(0,1)) + xlab('Similiarity') + ylab('AUC') + theme(axis.text.x=element_blank())
    } else if (plot_type == 'density') {
        gp <- ggplot(auc_dataframe, aes(x=auc, color=Similarity)) + geom_density() + facet_wrap(library~., scales='free_y', ncol=3) + theme_minimal() + scale_x_continuous(lim=c(0,1)) + xlab('AUC') + ylab('Density')
    }
